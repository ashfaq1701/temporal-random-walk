...
__global__ inline void empty_kernel(bool* result, const EdgeDataStore* edge_data) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        *result = edge_data::empty(edge_data);
    }
}

__global__ inline void size_kernel(size_t* result, const EdgeDataStore* edge_data) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        *result = edge_data::size(edge_data);
    }
}

__global__ inline void find_group_after_timestamp_kernel(size_t* result, const EdgeDataStore* edge_data, const int64_t timestamp) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        *result = edge_data::find_group_after_timestamp_device(edge_data, timestamp);
    }
}

__global__ inline void find_group_before_timestamp_kernel(size_t* result, const EdgeDataStore* edge_data, const int64_t timestamp) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        *result = edge_data::find_group_before_timestamp_device(edge_data, timestamp);
    }
}

__global__ inline void get_timestamp_group_range_kernel(SizeRange* result, const EdgeDataStore* edge_data, const size_t group_idx) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        *result = edge_data::get_timestamp_group_range(edge_data, group_idx);
    }
}

__global__ inline void get_timestamp_group_count_kernel(size_t* result, const EdgeDataStore* edge_data) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
...
public:
    ...
    explicit EdgeData(const bool use_gpu): owns_edge_data(true) {
        edge_data = new EdgeDataStore(use_gpu);
    }

    explicit EdgeData(EdgeDataStore* existing_edge_data) : edge_data(existing_edge_data), owns_edge_data(false) {}
    ...
    void set_size(const size_t size) const {
    ...
    void push_back(const int source, const int target, const int64_t timestamp) const {
        #ifdef HAS_CUDA
        if (edge_data->use_gpu) {
    ...
    void update_temporal_weights(const double timescale_bound) const {
        #ifdef HAS_CUDA
        if (edge_data->use_gpu) {
    ...
    [[nodiscard]] std::pair<size_t, size_t> get_timestamp_group_range(const size_t group_idx) const {
        #ifdef HAS_CUDA
        if (edge_data->use_gpu) {
            // Call via CUDA kernel for GPU implementation
            SizeRange* d_result;
    ...
    [[nodiscard]] size_t find_group_after_timestamp(const int64_t timestamp) const {
        #ifdef HAS_CUDA
        if (edge_data->use_gpu) {
            // Call via CUDA kernel for GPU implementation
            size_t* d_result;
    ...
    [[nodiscard]] size_t find_group_before_timestamp(const int64_t timestamp) const {
        #ifdef HAS_CUDA
        if (edge_data->use_gpu) {
            // Call via CUDA kernel for GPU implementation
            size_t* d_result;
