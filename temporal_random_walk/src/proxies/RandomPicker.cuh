...
public:
    ...
    explicit ExponentialIndexRandomPicker(const bool use_gpu): use_gpu(use_gpu) {}
...
public:
    ...
    explicit LinearRandomPicker(const bool use_gpu) : use_gpu(use_gpu) {}
...
public:
    explicit UniformRandomPicker(const bool use_gpu) : use_gpu(use_gpu) {}

    int pick_random(const int start, const int end, bool /* prioritize_end */) const {
        // Initialize rand nums between [0, 1)
        double* rand_nums = generate_n_random_numbers(1, use_gpu);
        int result;
