...
HOST void temporal_graph::sort_and_merge_edges_std(TemporalGraphStore *graph, const size_t start_idx) {
    ...
    const auto sorted_sources = new int[new_edges_count];
    const auto sorted_targets = new int[new_edges_count];
    const auto sorted_timestamps = new int64_t[new_edges_count];
    ...
    const auto merged_sources = new int[total_size];
    const auto merged_targets = new int[total_size];
    const auto merged_timestamps = new int64_t[total_size];
...
HOST void temporal_graph::sort_and_merge_edges_cuda(TemporalGraphStore *graph, const size_t start_idx) {
    ...
    using ZipIterator = thrust::zip_iterator<thrust::tuple<int*, int*>>;
    using ConstZipIterator = thrust::zip_iterator<thrust::tuple<const int*, const int*>>;
