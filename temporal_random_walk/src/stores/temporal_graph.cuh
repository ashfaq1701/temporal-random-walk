...
namespace temporal_graph {
    ...
    HOST inline void update_temporal_weights(const TemporalGraphStore* graph) {
        #ifdef HAS_CUDA
        if (graph->use_gpu) {
    ...
    HOST DEVICE inline size_t get_total_edges(const TemporalGraphStore* graph) {
        return edge_data::size(graph->edge_data);
    }

    HOST inline size_t get_node_count(const TemporalGraphStore* graph) {
        return edge_data::active_node_count(graph->edge_data);
    }

    HOST inline int64_t get_latest_timestamp(const TemporalGraphStore* graph) {
        return graph->latest_timestamp;
    }

    HOST inline DataBlock<int> get_node_ids(const TemporalGraphStore* graph) {
        return edge_data::get_active_node_ids(graph->edge_data);
    }

    HOST inline DataBlock<Edge> get_edges(const TemporalGraphStore* graph) {
    ...
    HOST inline void sort_and_merge_edges_std(TemporalGraphStore *graph, const size_t start_idx) {
    ...
    HOST inline void delete_old_edges_std(TemporalGraphStore* graph) {
    ...
    HOST inline size_t count_timestamps_greater_than_std(const TemporalGraphStore* graph, const int64_t timestamp) {
        if (edge_data::empty(graph->edge_data)) return 0;
    ...
    HOST inline size_t count_node_timestamps_less_than_std(TemporalGraphStore* graph, const int node_id, const int64_t timestamp) {
    ...
    HOST inline size_t count_node_timestamps_greater_than_std(TemporalGraphStore* graph, const int node_id, const int64_t timestamp) {
        if (!edge_data::is_node_active_host(graph->edge_data, node_id)) {
            return 0;
        }
    ...
    HOST inline void sort_and_merge_edges_cuda(TemporalGraphStore *graph, const size_t start_idx) {
        const size_t total_size = edge_data::size(graph->edge_data);
        const size_t new_edges_count = total_size - start_idx;
        if (new_edges_count == 0) return;
    ...
    HOST inline void delete_old_edges_cuda(TemporalGraphStore *graph) {
        if (graph->max_time_capacity <= 0 || edge_data::empty(graph->edge_data)) return;
    ...
    HOST inline size_t count_timestamps_less_than_cuda(const TemporalGraphStore* graph, const int64_t timestamp) {
        if (edge_data::empty(graph->edge_data)) return 0;
    ...
    HOST inline size_t count_timestamps_greater_than_cuda(const TemporalGraphStore* graph, const int64_t timestamp) {
        if (edge_data::empty(graph->edge_data)) return 0;
    ...
    HOST inline size_t count_node_timestamps_less_than_cuda(const TemporalGraphStore* graph, const int node_id, const int64_t timestamp) {
        if (!edge_data::is_node_active_host(graph->edge_data, node_id)) {
            return 0;
        }
    ...
    HOST inline size_t count_node_timestamps_greater_than_cuda(const TemporalGraphStore* graph, const int node_id, const int64_t timestamp) {
        if (!edge_data::is_node_active_host(graph->edge_data, node_id)) {
            return 0;
        }
    ...
    HOST inline TemporalGraphStore* to_device_ptr(const TemporalGraphStore* graph) {
    ...
    HOST inline void free_device_pointers(TemporalGraphStore* d_graph) {
        if (!d_graph) return;
