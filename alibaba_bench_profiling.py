"""Run test_alibaba_streaming under nsys, extract walk-side + ingest-side
metrics + per-bucket kernel breakdown, write three CSVs.

Each (variant, rep) is profiled once. The resulting .sqlite has both
`walk_sampling_batch` and `ingestion_batch` NVTX ranges; the kernels inside
those ranges drive all three extractors:
  ...
  - kernel split : launches + total_ms per bucket (solo, warp_smem, etc.)

Single arg: alibaba data dir. All other params hardcoded to the winning
Backward config used by alibaba_bench.py.

    python alibaba_bench_profiling.py /path/to/alibaba_dir

C++ stdout is captured (not echoed). Per-rep one-liners + per-variant
summaries print. Final three tables to stdout. CSVs:
  alibaba_profiling_walks.csv      one row per variant
  alibaba_profiling_ingest.csv     one pooled row (variant-agnostic)
  alibaba_profiling_kernels.csv    one row per (variant, bucket)
"""
import argparse
import csv
import re
...
# Positional CLI for test_alibaba_streaming, same as alibaba_bench.py.
...
def invoke_nsys(nsys_bin, rep_path, binary, data_dir, variant):
    args = COMMON.copy()
    args[2]  = variant
    args[-2] = ''  # no per-batch CSV needed for profiling
    cmd = [nsys_bin, 'profile', '--trace=cuda,nvtx', '--stats=true',
           '--force-overwrite=true',
           '--output', str(rep_path.with_suffix('')),
           str(binary), data_dir, *args]
...
def main():
    ...
    ap.add_argument('--binary',  default=str(Path(__file__).resolve().parent
                                             / 'build' / 'bin'
                                             / 'test_alibaba_streaming'))
    ap.add_argument('--nsys',    default='nsys')
    ap.add_argument('--rep-dir', default='alibaba_profiling_nsys')
    ap.add_argument('--out-prefix', default='alibaba_profiling')
    args = ap.parse_args()
    ...
    walk_runs   = {v: [] for v in VARIANTS}      # list of dicts per rep
    bucket_runs = {v: [] for v in VARIANTS}      # list of {bucket: {n,ms}}
    throughput  = {v: [] for v in VARIANTS}      # (walks/s, steps/s, avg_len)
    ingest_pool = []                             # variant-agnostic, pooled
    ...
    for variant in VARIANTS:
        ...
        for rep in range(args.reps):
            ...
            try:
                wps, sps, avg_len = invoke_nsys(
                    args.nsys, rep_path, binary, args.data_dir, variant)
        ...
        # Per-variant summary on the walk-side metrics.
    ...
    if ingest_pool:
        ...
        print('=' * 84)
        print(f' n_samples={r["n_samples"]}  total_ms={r["ingest_total_ms_mean"]:.2f}±'
              f'{r["ingest_total_ms_std"]:.2f}  '
              f'sort={r["ingest_sort_ms_mean"]:.2f}ms ({r["ingest_sort_frac"]*100:.1f}%)  '
              f'weight={r["ingest_weight_ms_mean"]:.2f}ms ({r["ingest_weight_frac"]*100:.1f}%)  '
              f'h2d={r["ingest_h2d_ms_mean"]:.2f}ms ({r["ingest_h2d_frac"]*100:.1f}%)  '
              f'launches={r["ingest_launches_mean"]:.1f}')
